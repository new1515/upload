...
if (isset($_POST['change_password'])) {
    ...
    if ($parent['password'] != $current) {
        ...
        $stmt = $pdo->prepare("UPDATE parents SET password = ? WHERE id = ?");
        $stmt->execute([md5($new), $parentId]);
...
<head>
    ...
    <style>
        ...
        .settings-card h3 i { color: #f39c12; }
        ...
        .btn-change { width: 100%; padding: 14px; background: linear-gradient(135deg, #f39c12, #d35400); color: white; border: none; border-radius: 10px; font-size: 15px; font-weight: 600; cursor: pointer; }
        .btn-change:hover { transform: translateY(-2px); box-shadow: 0 5px 20px rgba(243,156,18,0.4); }
