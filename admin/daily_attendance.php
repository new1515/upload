...
$selectedClass = isset($_POST['class_id']) ? sanitize($_POST['class_id']) : (isset($_GET['class']) ? sanitize($_GET['class']) : '');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_daily_attendance'])) {
    ...
    foreach ($studentIds as $studentId) {
        ...
        $stmt->execute([$studentId, $selectedDate, $status, $remark, $_SESSION['admin_id']]);
        $saved++;
    }
    
    logActivity($pdo, 'update', "Marked daily attendance for $saved students on $selectedDate", $_SESSION['admin_username'] ?? 'admin', 'daily_attendance', null);
...
if (isset($_GET['mark_all'])) {
    ...
    if (in_array($allStatus, ['present', 'absent', 'late'])) {
        ...
        foreach ($students as $student) {
            ...
            $stmt->execute([$student['id'], $selectedDate, $allStatus, $_SESSION['admin_id']]);
        }
        
        logActivity($pdo, 'update', "Marked all students as $allStatus on $selectedDate", $_SESSION['admin_username'] ?? 'admin', 'daily_attendance', null);
...
<head>
    ...
    <title>Daily Attendance Register - School Management System</title>
    ...
    <style>
        ...
        .quick-mark { display: flex; gap: 10px; margin-bottom: 15px; }
