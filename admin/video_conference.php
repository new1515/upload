...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .video-grid.count-7, .video-grid.count-8, .video-grid.count-9 { grid-template-columns: repeat(3, 1fr); max-width: 1000px; margin: 0 auto; }
        .video-grid.count-10, .video-grid.count-11, .video-grid.count-12 { grid-template-columns: repeat(4, 1fr); max-width: 1200px; margin: 0 auto; }
...
<body>
    <div class="meet-app" id="meetApp">
        <header class="meet-header">
            ...
            <button class="share-btn" id="shareBtn" onclick="toggleShareMenu()" style="display: none; padding: 8px 16px; border: none; border-radius: 20px; background: rgba(255,255,255,0.1); color: white; cursor: pointer; font-size: 13px; display: flex; align-items: center; gap: 8px;">
                ...
                <div class="video-scroll-container" id="videoScrollContainer">
                    <div class="video-grid" id="videoGrid">
                        <div class="waiting-screen" id="waitingScreen">
                            ...
                            <h2><?php echo $roomName ? htmlspecialchars($roomName) : 'Ready to Join'; ?></h2>
                            <p><?php echo $roomName ? 'Share the code with others to join' : 'Enter a room code to get started'; ?></p>
                            ...
                            <?php else: ?>
                            <div class="room-code-display">
                                <input type="text" id="roomCodeInput" placeholder="Enter room code" style="background: transparent; border: none; color: var(--primary); font-size: 24px; text-align: center; width: 200px;">
                            </div>
        ...
        <footer class="meet-controls" id="meetControls" style="display: none;">
            ...
            <button class="control-btn record" id="recordBtn" onclick="toggleRecording()" style="background: rgba(255,255,255,0.9); color: #333;">
    ...
    <div class="share-popup" id="sharePopup" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(30,30,46,0.98); border-radius: 16px; padding: 24px; width: 400px; max-width: 90%; z-index: 300; display: none; box-shadow: 0 20px 60px rgba(0,0,0,0.5);">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            <button onclick="toggleShareMenu()" style="background: none; border: none; color: white; font-size: 20px; cursor: pointer;">
        ...
        <div style="margin-bottom: 16px;">
            ...
            <div style="display: flex; gap: 8px;">
                ...
                <button onclick="copyMeetingLink()" style="padding: 12px 16px; border: none; border-radius: 8px; background: var(--primary); color: white; cursor: pointer;">
        ...
        <div style="margin-bottom: 16px;">
            ...
            <div style="display: flex; gap: 8px;">
                ...
                <button onclick="copyMeetingId()" style="padding: 12px 16px; border: none; border-radius: 8px; background: var(--primary); color: white; cursor: pointer;">
                    <i class="fas fa-copy"></i>
                </button>
            </div>
        </div>
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px 20px; background: var(--bg-card); border-bottom: 1px solid rgba(255,255,255,0.1);">
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                <div style="display: flex; gap: 4px; padding: 4px 8px; background: rgba(255,255,255,0.1); border-radius: 8px;">
                    <div class="color-swatch" onclick="setWhiteboardColor(this, '#000000')" style="width: 24px; height: 24px; border-radius: 50%; background: #000000; cursor: pointer; border: 2px solid white;"></div>
                    <div class="color-swatch" onclick="setWhiteboardColor(this, '#ea4335')" style="width: 24px; height: 24px; border-radius: 50%; background: #ea4335; cursor: pointer;"></div>
                    <div class="color-swatch" onclick="setWhiteboardColor(this, '#667eea')" style="width: 24px; height: 24px; border-radius: 50%; background: #667eea; cursor: pointer;"></div>
                    <div class="color-swatch" onclick="setWhiteboardColor(this, '#1ab26b')" style="width: 24px; height: 24px; border-radius: 50%; background: #1ab26b; cursor: pointer;"></div>
                    <div class="color-swatch" onclick="setWhiteboardColor(this, '#fbbc05')" style="width: 24px; height: 24px; border-radius: 50%; background: #fbbc05; cursor: pointer;"></div>
                </div>
    ...
    <script>
        ...
        function createMeeting() {
            fetch('meeting_api.php?action=create', {
                ...
                body: `host_id=<?php echo $_SESSION['admin_id'] ?? 0; ?>&host_name=${encodeURIComponent(currentUserName)}&host_role=${encodeURIComponent(currentUserRole)}&title=${encodeURIComponent(currentRoom || 'Video Meeting')}`
            })
            .then(r => r.json())
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('shareBtn').onclick = toggleShareMenu;
                }
                initMeeting();
            })
            .catch(() => {
        ...
        function pollWhiteboardStatus() {
            fetch('whiteboard_api.php?action=get')
                .then(r => r.json())
                .then(data => {
                    if (!isHost && data.is_open && data.opened_by && !whiteboardOpen) {
                        showWhiteboardNotification(data.opened_by);
                    }
                })
                .catch(() => {});
        }
        ...
        function updateSpeakingIndicator() {
            ...
            if (tile) {
                tile.classList.toggle('speaking', isSpeaking && !isMuted);
            }
            if (preview) {
                preview.classList.toggle('speaking', isSpeaking && !isMuted);
            }
        }
        ...
        function joinRoom() {
            ...
            if (code) {
                currentRoom = code;
                document.getElementById('waitingScreen').querySelector('h2').textContent = code.toUpperCase();
                document.getElementById('waitingScreen').querySelector('.room-code-display').innerHTML = `
                    <span class="code">${code.toUpperCase()}</span>
                    <button class="copy-btn" onclick="copyCode()"><i class="fas fa-copy"></i> Copy</button>
                `;
                document.getElementById('waitingScreen').querySelector('p').textContent = 'Share the code with others to join';
                document.querySelector('.join-btn').innerHTML = '<i class="fas fa-video"></i> Join Now';
                document.querySelector('.join-btn').onclick = startConference;
            }
        }
        ...
        function renderVideoGrid() {
            ...
            const allParticipants = [
                { id: 0, name: 'You', role: currentUserRole, isMuted: isMuted, isHost: isAdmin, isDummy: false, isYou: true, stream: localStream },
                ...participants
            ];
            ...
            allParticipants.forEach(p => {
                ...
                html += `
                    <div class="video-tile ${p.isMuted ? '' : 'speaking'}" id="tile-${p.id}">
                        ...
                        <div class="name-tag">
                            ${p.isYou ? '<i class="fas fa-user"></i>' : '<i class="fas fa-user"></i>'}
                            ${escapeHtml(p.name)}
                        ...
                        ${isAdmin && !p.isYou ? `
                            <div style="position: absolute; top: 12px; right: 50px; display: flex; gap: 8px;">
                                <button onclick="muteParticipant(${p.id})" style="width: 28px; height: 28px; border-radius: 50%; border: none; background: rgba(0,0,0,0.5); color: white; cursor: pointer;">
                                    <i class="fas fa-microphone${p.isMuted ? '' : '-slash'}"></i>
                                </button>
                                <button onclick="removeParticipant(${p.id})" style="width: 28px; height: 28px; border-radius: 50%; border: none; background: rgba(225,67,53,0.8); color: white; cursor: pointer;">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                        ` : ''}
            ...
            list.innerHTML = all.map(p => {
                ...
                return `
                    <div class="participant-item">
                        ...
                        <div class="icons">
                            ${p.isHost ? '<i class="fas fa-crown" style="color: #fbbc05;"></i>' : ''}
        ...
        function toggleCamera() {
            ...
            const preview = document.getElementById('localPreview');
            if (preview) {
                preview.style.display = isCameraOff ? 'none' : 'block';
            }
        ...
        function sendReaction(emoji) {
            ...
            fetch(`chat_api.php?action=reaction&emoji=${encodeURIComponent(emoji)}&sender=${encodeURIComponent(currentUserName)}`)
        ...
        function sendChatMessage() {
            ...
            if (text) {
                fetch(`chat_api.php?action=send&sender=${encodeURIComponent(currentUserName)}&message=${encodeURIComponent(text)}&role=${encodeURIComponent(currentUserRole)}`)
                    .then(r => r.json())
                    .then(data => {
        ...
        function copyMeetingId() {
            const id = document.getElementById('meetingIdDisplay').value;
        ...
        function sendMeetingLink() {
            const link = document.getElementById('meetingLink').value;
            const msg = `Join my meeting: ${link}`;
            fetch(`chat_api.php?action=send&sender=${encodeURIComponent(currentUserName)}&message=${encodeURIComponent(msg)}&role=${encodeURIComponent(currentUserRole)}`)
                .then(r => r.json())
                .then(data => {
                    if (data.success) {
                        alert('Link sent to chat!');
                        toggleShareMenu();
                    }
                })
                .catch(err => console.warn('Send link error:', err));
        }
        ...
        async function startRecording() {
            try {
                ...
                recordingStream = await navigator.mediaDevices.getDisplayMedia(displayOptions);
                
                const videoElement = document.createElement('video');
                videoElement.srcObject = recordingStream;
                videoElement.style.display = 'none';
                document.body.appendChild(videoElement);
                
                const combinedStream = videoElement.captureStream ? videoElement.captureStream() : recordingStream;
                ...
                if (currentMeetingId) {
                    fetch(`meeting_api.php?action=record`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        body: `meeting_id=${currentMeetingId}&record_action=start`
                    }).catch(() => {});
                }
            } catch (err) {
        ...
        function stopRecording() {
            ...
            if (currentMeetingId) {
                fetch(`meeting_api.php?action=record`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: `meeting_id=${currentMeetingId}&record_action=stop`
                }).catch(() => {});
            }
        }
        ...
        function toggleWhiteboard() {
            ...
            if (whiteboardOpen) {
                ...
                fetch('whiteboard_api.php?action=open&role=' + encodeURIComponent(currentUserRole) + '&name=' + encodeURIComponent(currentUserName));
            } else {
                modal.style.display = 'none';
                btn.classList.remove('active');
                stopWhiteboardPolling();
                fetch('whiteboard_api.php?action=close&role=' + encodeURIComponent(currentUserRole));
            }
        }
        ...
        function setWhiteboardColor(el, color) {
        ...
        function stopDrawing() {
            if (isDrawing && currentStroke.length > 1 && isHost) {
        ...
        function setTool(tool) {
            currentTool = tool;
            document.getElementById('penTool').style.background = tool === 'pen' ? 'var(--primary)' : 'rgba(255,255,255,0.1)';
            document.getElementById('eraserTool').style.background = tool === 'eraser' ? 'var(--primary)' : 'rgba(255,255,255,0.1)';
        }
        
        function sendWhiteboardStroke(stroke) {
            fetch('whiteboard_api.php?action=add_stroke&role=' + encodeURIComponent(currentUserRole), {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        ...
        function clearWhiteboard() {
            ...
            fetch('whiteboard_api.php?action=clear&role=' + encodeURIComponent(currentUserRole))
                .catch(() => {});
        }
        ...
        function loadWhiteboard() {
            ...
            fetch('whiteboard_api.php?action=get_all')
                .then(r => r.json())
                .then(data => {
                    if (data.strokes) {
        ...
        function pollWhiteboard() {
            fetch('whiteboard_api.php?action=get&since=' + lastWhiteboardUpdate)
                .then(r => r.json())
                .then(data => {
        ...
        function showWhiteboardNotification(hostName) {
            const btn = document.getElementById('whiteboardBtn');
            btn.style.animation = 'pulse 1s infinite';
            btn.title = hostName + ' is presenting on whiteboard';
            if (!document.getElementById('whiteboardNotif')) {
                const notif = document.createElement('div');
                notif.id = 'whiteboardNotif';
                notif.style.cssText = 'position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:var(--success);color:white;padding:12px 24px;border-radius:8px;font-size:14px;z-index:250;animation:fadeIn 0.3s;';
                notif.innerHTML = '<i class="fas fa-pen-fancy"></i> ' + hostName + ' is presenting on whiteboard';
                notif.onclick = function() { toggleWhiteboard(); this.remove(); };
        ...
        function pollChat() {
            fetch(`chat_api.php?action=get&since=${lastChatUpdate}`)
                .then(r => r.json())
                .then(data => {
        ...
        function notifyJoin() {
            fetch(`chat_api.php?action=join&sender=${encodeURIComponent(currentUserName)}&role=${encodeURIComponent(currentUserRole)}`)
                .catch(err => console.warn('Join notification error:', err));
        }
        
        function notifyLeave() {
            fetch(`chat_api.php?action=leave&sender=${encodeURIComponent(currentUserName)}&role=${encodeURIComponent(currentUserRole)}`)
