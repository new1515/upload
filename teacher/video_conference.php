...
$error = '';
$roomName = '';
$userName = $_SESSION['teacher_name'] ?? 'Teacher';
$userRole = 'teacher';
$isAdmin = false;
$isHost = true;
$currentMeetingId = '';
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .meet-timer.recording .dot { background: var(--danger); }
        ...
        .control-btn.screen { background: rgba(255,255,255,0.9); color: #333; }
        .control-btn.screen.active { background: var(--warning); color: #333; }
        
        .control-btn.record { background: rgba(255,255,255,0.9); color: #333; }
        .control-btn.record.active { background: var(--danger); color: white; }
        ...
        .join-btn { background: var(--success); }
        .create-btn { background: var(--primary); }
        
        .join-btn:hover, .create-btn:hover { transform: scale(1.05); }
...
<body>
    <div class="meet-app">
        <header class="meet-header">
            ...
            <button class="share-btn" id="shareBtn" onclick="toggleShareMenu()" style="display: none;">
                ...
                <div class="video-scroll-container" id="videoScrollContainer">
                    <div class="video-grid" id="videoGrid">
                        <div class="waiting-screen" id="waitingScreen">
                            <div class="room-icon"><i class="fas fa-chalkboard-teacher"></i></div>
                            <h2><?php echo $roomName ? htmlspecialchars($roomName) : 'Create or Join'; ?></h2>
                            <p><?php echo $roomName ? 'Share the code with students' : 'Start a new meeting'; ?></p>
                            
                            <?php if ($roomName): ?>
                            <div class="room-code-display">
                                <span class="code"><?php echo strtoupper($roomName); ?></span>
                                <button class="create-btn" onclick="startConference()">
                            ...
                            <div class="room-code-display">
                                ...
                                <input type="text" id="roomCodeInput" placeholder="Enter code to join">
        ...
        <footer class="meet-controls" id="meetControls" style="display: none;">
            ...
            <button class="control-btn screen" id="screenBtn" onclick="toggleScreen()">
                <i class="fas fa-desktop"></i>
                <span class="tooltip">Share screen</span>
            </button>
            
            <button class="control-btn record" id="recordBtn" onclick="toggleRecording()">
    ...
    <div class="share-popup" id="sharePopup" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(30,30,46,0.98); border-radius: 16px; padding: 24px; width: 400px; max-width: 90%; z-index: 300; display: none; box-shadow: 0 20px 60px rgba(0,0,0,0.5);">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <h3 style="margin: 0;">Share meeting</h3>
            <button onclick="toggleShareMenu()" style="background: none; border: none; color: white; font-size: 20px; cursor: pointer;"><i class="fas fa-times"></i></button>
        </div>
        <div style="margin-bottom: 16px;">
            <label style="display: block; font-size: 12px; color: var(--text-muted); margin-bottom: 8px;">Meeting link</label>
            <div style="display: flex; gap: 8px;">
                <input type="text" id="meetingLink" readonly style="flex: 1; padding: 12px; border: none; border-radius: 8px; background: rgba(255,255,255,0.1); color: white; font-size: 13px;">
                <button onclick="copyMeetingLink()" style="padding: 12px 16px; border: none; border-radius: 8px; background: var(--primary); color: white; cursor: pointer;"><i class="fas fa-copy"></i></button>
            </div>
        </div>
        <div style="margin-bottom: 16px;">
            <label style="display: block; font-size: 12px; color: var(--text-muted); margin-bottom: 8px;">Meeting ID</label>
            <div style="display: flex; gap: 8px;">
                <input type="text" id="meetingIdDisplay" readonly style="flex: 1; padding: 12px; border: none; border-radius: 8px; background: rgba(255,255,255,0.1); color: var(--primary); font-size: 18px; font-weight: 600; letter-spacing: 2px; text-align: center;">
                <button onclick="copyMeetingId()" style="padding: 12px 16px; border: none; border-radius: 8px; background: var(--primary); color: white; cursor: pointer;"><i class="fas fa-copy"></i></button>
            </div>
        </div>
        <button onclick="sendMeetingLink()" style="width: 100%; padding: 12px; border: none; border-radius: 8px; background: rgba(255,255,255,0.1); color: white; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px;">
            <i class="fas fa-envelope"></i> Send link via chat
        </button>
    </div>
    ...
    <div class="whiteboard-modal" id="whiteboardModal" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: white; z-index: 300; display: none;">
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px 20px; background: var(--bg-card); border-bottom: 1px solid rgba(255,255,255,0.1);">
            <h3 style="display: flex; align-items: center; gap: 10px; font-size: 16px; font-weight: 500; margin: 0; color: var(--text-light);"><i class="fas fa-pen-fancy" style="color: var(--primary);"></i> Whiteboard</h3>
            <div style="display: flex; align-items: center; gap: 8px;">
                <div style="display: flex; gap: 4px; padding: 4px 8px; background: rgba(255,255,255,0.1); border-radius: 8px;">
                    <div onclick="setWhiteboardColor(this, '#000000')" style="width: 24px; height: 24px; border-radius: 50%; background: #000000; cursor: pointer; border: 2px solid white;"></div>
                    <div onclick="setWhiteboardColor(this, '#ea4335')" style="width: 24px; height: 24px; border-radius: 50%; background: #ea4335; cursor: pointer;"></div>
                    <div onclick="setWhiteboardColor(this, '#667eea')" style="width: 24px; height: 24px; border-radius: 50%; background: #667eea; cursor: pointer;"></div>
                    <div onclick="setWhiteboardColor(this, '#1ab26b')" style="width: 24px; height: 24px; border-radius: 50%; background: #1ab26b; cursor: pointer;"></div>
                </div>
                <input type="range" id="brushSize" min="1" max="20" value="3" style="width: 80px;">
                <button onclick="setTool('pen')" id="penTool" style="width: 40px; height: 40px; border-radius: 8px; border: none; background: var(--primary); color: white; cursor: pointer;"><i class="fas fa-pen"></i></button>
                <button onclick="setTool('eraser')" id="eraserTool" style="width: 40px; height: 40px; border-radius: 8px; border: none; background: rgba(255,255,255,0.1); color: white; cursor: pointer;"><i class="fas fa-eraser"></i></button>
                <button onclick="clearWhiteboard()" style="width: 40px; height: 40px; border-radius: 8px; border: none; background: rgba(255,255,255,0.1); color: white; cursor: pointer;"><i class="fas fa-trash"></i></button>
            </div>
            <button onclick="toggleWhiteboard()" style="width: 40px; height: 40px; border-radius: 50%; border: none; background: var(--danger); color: white; cursor: pointer;"><i class="fas fa-times"></i></button>
        </div>
        <canvas id="whiteboardCanvas" style="display: block; width: 100vw; height: calc(100vh - 64px); background: white; cursor: crosshair;"></canvas>
    </div>
    
    <script>
        let localStream = null, screenStream = null;
        let isMuted = false, isCameraOff = false, isHandRaised = false;
        let isScreenSharing = false, isInCall = false;
        ...
        let currentMeetingId = '<?php echo htmlspecialchars($currentMeetingId); ?>';
        let isHost = <?php echo $isHost ? 'true' : 'false'; ?>;
        
        let whiteboardOpen = false;
        let whiteboardCanvas = null, whiteboardCtx = null;
        let isDrawing = false, currentTool = 'pen', currentColor = '#000000';
        let brushSize = 3, lastX = 0, lastY = 0, currentStroke = [];
        let whiteboardPollInterval = null, lastWhiteboardUpdate = 0;
        ...
        function createMeeting() {
            fetch('../meeting_api.php?action=create', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `host_id=<?php echo $_SESSION['teacher_id'] ?? 0; ?>&host_name=${encodeURIComponent(currentUserName)}&host_role=${encodeURIComponent(currentUserRole)}&title=${encodeURIComponent(currentRoom || 'Video Meeting')}`
            })
            .then(r => r.json())
            .then(data => {
                if (data.success) {
                    currentRoom = data.meeting_id;
        ...
        function initMeeting() {
            ...
            setInterval(pollWhiteboardStatus, 1000);
            
            if (currentMeetingId) {
                fetch('../meeting_api.php?action=start', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        ...
        function createRoom() {
            ...
            if (name) {
                ...
                form.method = 'POST';
                form.innerHTML = '<input name="create_room" value="1"><input name="room_name" value="'+name+'">';
        ...
        function simulateParticipants() {
            participants = [];
            const names = ['Student', 'Student', 'Student', 'Parent'];
            const roles = ['student', 'student', 'student', 'parent'];
        ...
        function renderVideoGrid() {
            ...
            const all = [
                { id: 0, name: 'You', role: currentUserRole, isMuted: isMuted, isHost: true, isDummy: false, isYou: true, stream: localStream },
                ...participants
            ];
                ...
                html += `
                    <div class="video-tile ${p.isMuted ? '' : 'speaking'}" id="tile-${p.id}">
                        ...
                        <div class="name-tag">
                            ...
                            ${p.isHost && p.isYou ? '<span class="badge badge-host">HOST</span>' : ''}
            ...
            list.innerHTML = all.map(p => {
                ...
                return `
                    <div class="participant-item">
                        ...
                        <div class="icons">
                            ${p.isHost && p.isYou ? '<i class="fas fa-crown" style="color: #fbbc05;"></i>' : ''}
        ...
        function toggleScreen() {
            if (isScreenSharing) {
                ...
                navigator.mediaDevices.getDisplayMedia({ video: true })
                    ...
                    .catch(err => console.warn('Screen share error:', err.message));
            }
        }
        ...
        function toggleShareMenu() {
            ...
            if (popup.style.display === 'none') {
                document.getElementById('meetingLink').value = window.location.origin + '/meeting.php?id=' + currentRoom;
                document.getElementById('meetingIdDisplay').value = currentRoom.toUpperCase();
        ...
        function copyMeetingLink() {
            navigator.clipboard.writeText(document.getElementById('meetingLink').value);
            alert('Meeting link copied!');
        }
        
        function copyMeetingId() {
            navigator.clipboard.writeText(document.getElementById('meetingIdDisplay').value);
            alert('Meeting ID copied!');
        }
        
        function sendMeetingLink() {
            const link = document.getElementById('meetingLink').value;
            fetch(`../admin/chat_api.php?action=send&sender=${encodeURIComponent(currentUserName)}&message=${encodeURIComponent('Join my meeting: ' + link)}&role=${encodeURIComponent(currentUserRole)}`)
                .then(r => r.json()).then(d => { if (d.success) { alert('Link sent!'); toggleShareMenu(); } }).catch(() => {});
        }
        ...
        function toggleRecording() {
            ...
            if (isRecording) {
                ...
                if (currentMeetingId) fetch(`../meeting_api.php?action=record`, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: `meeting_id=${currentMeetingId}&record_action=start` }).catch(() => {});
            } else {
                btn.classList.remove('active');
                btn.querySelector('i').className = 'fas fa-record-vinyl';
                indicator.style.display = 'none';
                timer.classList.remove('recording');
                if (currentMeetingId) fetch(`../meeting_api.php?action=record`, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: `meeting_id=${currentMeetingId}&record_action=stop` }).catch(() => {});
            }
        }
        ...
        function toggleWhiteboard() {
            ...
            if (whiteboardOpen) { modal.style.display = 'flex'; btn.classList.add('active'); closeAllPanels(); initWhiteboard(); startWhiteboardPolling(); fetch('whiteboard_api.php?action=open&role=' + encodeURIComponent(currentUserRole) + '&name=' + encodeURIComponent(currentUserName)); }
            else { modal.style.display = 'none'; btn.classList.remove('active'); stopWhiteboardPolling(); fetch('whiteboard_api.php?action=close&role=' + encodeURIComponent(currentUserRole)); }
        }
        ...
        function setWhiteboardColor(el, color) { document.querySelectorAll('[onclick^="setWhiteboardColor"]').forEach(s => s.style.border = 'none'); el.style.border = '2px solid white'; currentColor = color; setTool('pen'); }
        function startDrawing(e) { isDrawing = true; const rect = whiteboardCanvas.getBoundingClientRect(); lastX = e.clientX - rect.left; lastY = e.clientY - rect.top; currentStroke = [{ x: lastX, y: lastY }]; }
        function draw(e) {
            if (!isDrawing) return;
            ...
            whiteboardCtx.beginPath(); whiteboardCtx.moveTo(lastX, lastY); whiteboardCtx.lineTo(x, y); whiteboardCtx.stroke();
            currentStroke.push({ x, y }); lastX = x; lastY = y;
        }
        function stopDrawing() { if (isDrawing && currentStroke.length > 1 && isHost) sendWhiteboardStroke({ points: currentStroke, color: currentTool === 'eraser' ? '#ffffff' : currentColor, size: currentTool === 'eraser' ? brushSize * 3 : brushSize }); isDrawing = false; currentStroke = []; }
        function setTool(tool) { currentTool = tool; document.getElementById('penTool').style.background = tool === 'pen' ? 'var(--primary)' : 'rgba(255,255,255,0.1)'; document.getElementById('eraserTool').style.background = tool === 'eraser' ? 'var(--primary)' : 'rgba(255,255,255,0.1)'; }
        function sendWhiteboardStroke(stroke) { fetch('whiteboard_api.php?action=add_stroke&role=' + encodeURIComponent(currentUserRole), { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'stroke=' + encodeURIComponent(JSON.stringify(stroke)) }).catch(() => {}); }
        function clearWhiteboard() { if (!isHost) return; if (!confirm('Clear the whiteboard?')) return; whiteboardCtx.fillStyle = '#ffffff'; whiteboardCtx.fillRect(0, 0, whiteboardCanvas.width, whiteboardCanvas.height); fetch('whiteboard_api.php?action=clear&role=' + encodeURIComponent(currentUserRole)).catch(() => {}); }
        function loadWhiteboard() { whiteboardCtx.fillStyle = '#ffffff'; whiteboardCtx.fillRect(0, 0, whiteboardCanvas.width, whiteboardCanvas.height); fetch('whiteboard_api.php?action=get_all').then(r => r.json()).then(data => { if (data.strokes) data.strokes.forEach(stroke => { if (stroke.points && stroke.points.length > 1) drawStroke(stroke); }); lastWhiteboardUpdate = data.last_update || 0; }).catch(() => {}); }
        function drawStroke(stroke) { if (!stroke.points || stroke.points.length < 2) return; whiteboardCtx.strokeStyle = stroke.color || '#000000'; whiteboardCtx.lineWidth = stroke.size || 3; whiteboardCtx.beginPath(); whiteboardCtx.moveTo(stroke.points[0].x, stroke.points[0].y); for (let i = 1; i < stroke.points.length; i++) whiteboardCtx.lineTo(stroke.points[i].x, stroke.points[i].y); whiteboardCtx.stroke(); }
        function pollWhiteboard() { fetch('whiteboard_api.php?action=get&since=' + lastWhiteboardUpdate).then(r => r.json()).then(data => { if (data.strokes) data.strokes.forEach(stroke => drawStroke(stroke)); if (data.last_update > lastWhiteboardUpdate) lastWhiteboardUpdate = data.last_update; if (!isHost && data.is_open && data.opened_by && !whiteboardOpen) showWhiteboardNotification(data.opened_by); }).catch(() => {}); }
        function showWhiteboardNotification(hostName) { const btn = document.getElementById('whiteboardBtn'); btn.style.animation = 'pulse 1s infinite'; btn.title = hostName + ' is presenting on whiteboard'; if (!document.getElementById('whiteboardNotif')) { const notif = document.createElement('div'); notif.id = 'whiteboardNotif'; notif.style.cssText = 'position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:var(--success);color:white;padding:12px 24px;border-radius:8px;font-size:14px;z-index:250;cursor:pointer;'; notif.innerHTML = '<i class="fas fa-pen-fancy"></i> ' + hostName + ' is presenting on whiteboard'; notif.onclick = function() { toggleWhiteboard(); this.remove(); }; document.body.appendChild(notif); setTimeout(() => notif.remove(), 5000); } }
        function startWhiteboardPolling() { if (whiteboardPollInterval) return; whiteboardPollInterval = setInterval(pollWhiteboard, 500); }
        function stopWhiteboardPolling() { if (whiteboardPollInterval) { clearInterval(whiteboardPollInterval); whiteboardPollInterval = null; } }
        function pollWhiteboardStatus() { fetch('whiteboard_api.php?action=get').then(r => r.json()).then(data => { if (!isHost && data.is_open && data.opened_by && !whiteboardOpen) showWhiteboardNotification(data.opened_by); }).catch(() => {}); }
